...
const inputSchema = z.object({
  ...
  from: z.string().datetime().optional().describe("Optional ISO date-time to start listing from"),
  to: z.string().datetime().optional().describe("Optional ISO date-time upper bound"),
});

async function execute({limit, from, to}: z.output<typeof inputSchema>, agent: Agent) {
