...
function execute({
  ...
  if (!currentEvent)
    return "No calendar event is currently selected.\nUse /calendar event select to choose an event.";
  ...
  if (currentEvent.attendees?.length)
    lines.push(
      `Attendees: ${currentEvent.attendees.map((attendee) => attendee.name ?? attendee.email).join(", ")}`,
    );
  if (currentEvent.description)
    lines.push(`Description: ${currentEvent.description}`);
  if (currentEvent.url) lines.push(`URL: ${currentEvent.url}`);
  if (currentEvent.meetingUrl)
    lines.push(`Meeting URL: ${currentEvent.meetingUrl}`);
  return lines.join("\n");
}
